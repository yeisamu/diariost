var FormsSelect = {
	...
	selectizeRemote2: function () {
        $('.selectize-remote2').selectize({
                  theme: 'repositories2',
                    valueField: 'id_prop',
                    labelField: 'nombre',
                    searchField: 'nombre',
                    ...
                    render: {
                      option: function(item, escape) {
                        ...
                        return '<div>' +
                          '<span class="title2">' +
                            '<span class="name2">'+escape(item.nombre) + '</span></span></div>';
                      }
                    },
                    score: function(search) {
                      var score = this.getScoreFunction(search);
                      return function(item) {
                        return score(item) * (1 + Math.min(item.nfila / 100, 1));
                      };
                    },
                  load: function(query, callback) {
                    if (!query.length) return callback();
                    ...
                    $.ajax({
                      url: url+'sart.php/sistemasart/buscacri?id_prop='+ encodeURIComponent(query),
                      dataType: 'json',
